...
1. [개요](#개요)
2. [IoT & 하드웨어 통신](#iot--하드웨어-통신) — 라이브모드 하드웨어 보드 및 핸드폰 카메라 포함 ⭐ v1.6
3. [AI & 머신러닝](#ai--머신러닝)
4. [컴퓨터 비전 & 인식](#컴퓨터-비전--인식)
5. [MLOps 파이프라인](#mlops-파이프라인--new-v16) ⭐ NEW (v1.6)
6. [데이터 과학 & 시각화](#데이터-과학--시각화)
7. [기존 확장 개선](#기존-확장-개선)
8. [전체 확장 목록](#전체-확장-목록)
...
### 4a. 라이브모드 하드웨어 보드 ⭐ NEW (v1.6)

실물 보드 4종을 브릭셀AI에서 USB 시리얼·블루투스(BLE)로 **라이브 제어**할 수 있습니다. 보드는 가벼운 **펌웨어 명령 디스패처**만 돌리고, 모든 로직은 Scratch VM에 있습니다(줄 단위 텍스트 프로토콜, `\n` 종단). 펌웨어는 확장에서 바로 굽거나 내려받을 수 있습니다.

| 보드 | EXT ID | 배선 방식 | 펌웨어 | 굽는 방법 |
| --- | --- | --- | --- | --- |
| **Rich Shield (Uno)** | `richshield` | 자매 보드, 고정 핀 | Arduino (라이브 디스패처) | Web Serial 굽기 |
| **Mega SuperRich** | `superrich` | 장치별 고정 핀 | Arduino (라이브 디스패처) | Web Serial 굽기 |
| **micro:bit V2 + ma:bit** | `microbitv2` | 실드, 고정 핀 | MakeCode (TypeScript) | MICROBIT 드라이브에 hex 드래그 |
| **ESP32 풀키트** | `esp32fullset` | 자유 배선 (핀번호 인자) | Arduino-ESP32 | esptool-js 자동 굽기 |

**공통 기능:**
- **듀얼 채널:** USB 시리얼(유선) 또는 블루투스 BLE(무선)로 연결 — 두 채널 모두 같은 프로토콜
- **펌웨어 버전 표시** + 연결 시 자동 핸드셰이크 (HELLO/CAPS)
- 보드별 입출력 전 계층 지원: 서보, 네오픽셀, RGB LED, 팬/모터, 부저/스피커, LCD/OLED, 도트매트릭스, 초음파, 각종 센서 등
- **micro:bit V2 전용:** 네오픽셀 LED 개수 설정, MakeCode 음정, 햇 이벤트(버튼/제스처/소리/로고/핀), 나침반 캘리브레이션 가드. BLE는 micro:bit 변형 NUS 사용 (TX = indicate, 특성 속성으로 자동 판별)
- **ESP32 풀키트 전용:** 고정 핀이 아니라 핀번호 인자로 자유 배선, 학생별 BLE 이름 (NVS), 클릭 입력 격자를 갖춘 8×8/8×16 도트매트릭스, 스크롤 문자, 부저 음정/박자 메뉴
- **슈퍼리치/리치실드 전용:** 드롭다운 메뉴로 고르는 MP3 통합 블록 1개 (정지/일시정지/이어하기/다음/이전/반복/음량±)

**사용 예시 (ESP32 풀키트):**
```
ESP32 시리얼(USB) 연결하기
180도서보(MG90S) 핀 32 각도 90
네오픽셀 핀 18 전체 R255 G0 B0
초음파 거리 센서 Echo 18 Trig 5 (cm)
```
...
**주요 기술:** WebRTC P2P + 로컬 브리지 + QR 페어링

**기능:**
- **핸드폰 카메라**를 Scratch 무대의 영상 소스로 주입
- 컴퓨터 화면에 뜬 QR 코드를 폰으로 찍으면 → WebRTC 1:1 연결 (학생별 룸 ID)
- **모든 AI 비전 확장과 자동 연동** — 얼굴/손/자세/사물 인식, 이미지 분류 등이 웹캠 대신 폰 영상을 사용 (공유 `runtime.ioDevices.video` 프로바이더 + CameraManager 동기화)
- 완전 분산: 학생마다 자기 폰이 자기 노트북에만 연결 (1:1, 중앙 서버 없음) — 30명 학급에서도 안전
...
### 9. 브릭셀AI TTS (BrixelAI Text-to-Speech)
...
- 손 스켈레톤 표시
- ⭐ **KNN 제스처 학습** — 나만의 손 제스처를 학습시켜 실시간으로 인식
...
### 19. 자율주행 비전 (Lane Recognition)

**주요 기술:** 영상처리 + PID 제어

**기능:**
- 양차선 중앙 인식 (검정/흰색 선)
- 단일 라인 추적 및 라인 위치 추적 (-100 ~ 100)
- 모터 속도 계산 (왼쪽/오른쪽)
- 조향각 계산
- 인식된 차선 수 리포터
- PID 제어기 내장 (조향값 계산)
...
**주요 블록 (22개):**
- `카메라 켜기` / `카메라 끄기`
- `양차선 중앙인식 시작 (차선 색상: [COLOR])` / `양차선 중앙인식 중지`
- `라인 추적 시작 (선: [COLOR] 임계값: [TH])` / `라인 추적 중지`
- `라인 위치 (-100 ~ 100)` / `인식된 차선 수` / `두 차선 중앙 오차값 (-1 ~ 1)`
- `왼쪽 모터 속도 (기본: [SPEED])` / `오른쪽 모터 속도 (기본: [SPEED])`
- `조향각 (중립: [CENTER] 범위: [RANGE])`
- `PID 설정 비례(Kp):[KP] 적분(Ki):[KI] 미분(Kd):[KD]` / `PID 초기화`
- `오버레이 보이기` / `오버레이 숨기기`
- `< 차선이 감지되었는가? >`
...
### 20. 손글씨 인식 (Handwriting Recognition)
...
**기능:**
- 웹캠으로 사물을 촬영하여 라벨을 붙여 실시간 이미지 분류 학습
- MobileNet v1 특징 추출 + KNN 분류
- 1회 학습, 연속 학습 모드
- 학습 데이터 JSON 파일 저장/불러오기
- 전후면 카메라 전환, 좌우반전
- BLE 전송용 클래스별 자동 ID 부여

**주요 블록 (28개):**
- `카메라 켜기` / `카메라 끄기`
- `MobileNet 모델 로드`
- `KNN [NAME] (으)로 1회 학습` / `KNN [NAME] (으)로 연속 학습 시작` / `KNN 연속 학습 중지`
- `KNN [NAME] 학습 데이터 삭제` / `KNN 모든 학습 데이터 초기화`
- `KNN 인식 시작` / `KNN 인식 중지`
- `KNN 이미지가 [NAME] (으)로 인식되었을 때` / `KNN 인식 결과가 바뀌었을 때`
- `KNN 인식 결과` / `KNN 인식 결과 ID` / `KNN 인식 정확도`
- `KNN [NAME] 학습 데이터 수` / `KNN 학습된 클래스 목록`
- `KNN 학습 데이터 저장하기` / `KNN 학습 데이터 불러오기`
- `카메라 [전면/후면] 전환` / `카메라 좌우반전 [켜기/끄기]`

**사용 예시:**
```
카메라 켜기
MobileNet 모델 로드
// 'A' 키를 누르면 "사과", 'B' 키를 누르면 "바나나" 학습
KNN [사과] (으)로 1회 학습
KNN 인식 시작
// 인식 결과가 실시간으로 표시됨
```
...
**기능:**
- 웹캠으로 COCO 80가지 사물 실시간 인식
- 스크래치 무대 위 바운딩박스 오버레이
- 사물 추적 추가/해제
- 타겟 위치(X, Y), 크기(너비, 높이), 정확도 리포터
- 10fps 프레임 제한 추론으로 성능 최적화
- 인식 기준값(정확도) 조절

**주요 블록 (30개):**
- `카메라 켜기` / `카메라 끄기`
- `EfficientDet 모델 로드` / `COCO 80 사물 목록 보기`
- `[OBJECT] 추적 추가` / `[NAME] 직접 입력 추적 추가` / `[OBJECT] 추적 해제` / `모든 추적 초기화`
- `정확도 기준값 [NUM]% 로 설정`
- `사물 인식 시작` / `사물 인식 중지`
- `바운딩박스 오버레이 [켜기/끄기]`
- `[OBJECT] 이(가) 감지되었을 때` / `타겟이 사라졌을 때` / `모델 준비되었을 때`
- `타겟 이름` / `타겟의 X좌표` / `타겟의 Y좌표` / `타겟의 너비` / `타겟의 높이` / `타겟의 정확도`
- `감지된 [OBJECT] 개수` / `감지된 전체 사물 수` / `현재 추적 사물 목록`
- `< 모델 준비 완료? >` / `< 인식 중? >` / `< [OBJECT] 감지됨? >`
...
[사람 (person)] 추적 추가
사물 인식 시작
만약 < [사람 (person)] 감지됨? > 이라면
  (타겟 이름) 을(를) X: (타겟의 X좌표) 에서 찾았다고 말하기
```
...
- `분류 시작` / `분류 중지`
- `분류 결과` / `확신도 (%)`
- `클래스 이미지 갤러리 보기` ⭐ NEW — 각 클래스에 추가한 이미지를 눈으로 확인 (96×96 썸네일, 클릭하면 원본 크기, JPG 다운로드)
- `모델 저장하기` / `모델 파일 불러오기`
...
- `선형 회귀 학습하기 (X: [X_COL], Y: [Y_COL])` / `선형 회귀로 예측하기`
- `KNN 학습하기` / `KNN으로 예측하기`
- `K-means 군집화: [COLS]를 [K]개 그룹으로 나누기`
...
## MLOps 파이프라인 ⭐ NEW (v1.6)

머신러닝의 전 과정을 8단계로 나눈 엔드투엔드 워크플로로, 브릭셀AI를 ML 생애주기 전체를 가르치는 플랫폼으로 만들어 줍니다. 각 단계는 별도의 확장이며, 공유 상태인 `runtime.brixelMLState`(현재 데이터셋, 현재 모델, 실험 기록, 배포된 모델)를 통해 서로 연결됩니다.

| 단계 | EXT ID | 역할 |
| --- | --- | --- |
| **1. 데이터 파이프라인** | `datapipeline` | 표 형태 데이터셋 만들기/정제, CSV 가져오기, 특징 열 지정 |
| **2. 미디어 파이프라인** | `mediapipeline` | 카메라나 파일로 이미지/오디오/얼굴/손/자세 데이터셋 만들기 |
| **3. AutoML** | `automl` | 데이터셋에 대해 모델 자동 선택 및 학습 |
| **4. 신경망 빌더** | `nnbuilder` | 신경망 구조를 층 단위로 직접 설계 |
| **5. 실험 추적** | `exptracking` | 실행 기록·하이퍼파라미터·지표를 남겨 비교 |
| **6. 모델 평가** | `modeleval` | `model.predict()`로 정확도, 혼동행렬, 클래스별 지표 산출 |
| **7. 책임성 AI** | `responsibleai` | 집단별 공정성·편향 점검 |
| **8. 모델 허브** | `modelhub` | 학습한 모델 공유/내려받기(Firebase), 경량 JSON 직렬화 |

**흐름:** 미디어/데이터 파이프라인 → 학습(AutoML / 신경망 빌더 / 학습 확장) → `모델을 파이프라인에 전달` → 모델 평가 → 책임성 AI → 모델 허브에 공개. 학습 확장(이미지/소리/텍스트 분류모델, KNN, SVM 등)은 학습한 모델을 `currentModel`로 전달하여 이 파이프라인에 연결됩니다.

> 참고: 모델 허브는 모델 가중치와 특징만 직렬화합니다(공유 모델을 가볍게 유지하기 위해 업로드 시 썸네일과 용량이 큰 원본 미디어는 제거됩니다).
...
- WebSocket (ws:// / wss://)
- WebRTC P2P (핸드폰 카메라)
- 라이브모드 펌웨어 디스패처 (Arduino, ESP32-Arduino, micro:bit MakeCode) + esptool-js 굽기
